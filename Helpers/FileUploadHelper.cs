namespace JobPortal.Helpers
{
    public static class FileUploadHelper
    {
        ...
        public static (bool isValid, string? errorMessage) ValidateDocument(IFormFile? document)
        {
            if (document == null || document.Length == 0)
                return (false, "Please upload a document file.");
        ...
        public static async Task<(bool success, string? filePath, string? errorMessage)> SaveDocumentAsync(
            IFormFile document,
            string webRootPath)
        {
            try
            {
                string uploadsFolder = Path.Combine(webRootPath, DOCUMENTS_FOLDER);
                if (!Directory.Exists(uploadsFolder))
                    Directory.CreateDirectory(uploadsFolder);
                ...
                return (true, $"/{DOCUMENTS_FOLDER}/{uniqueFileName}", null);
            }
            catch (Exception ex)
            {
                return (false, null, $"File upload error: {ex.Message}");
        ...
        public static bool DeleteDocument(string documentPath, string webRootPath)
        {
            try
            {
                ...
                string filePath = Path.Combine(webRootPath, documentPath.TrimStart('/'));
                if (File.Exists(filePath))
                {
                    File.Delete(filePath);
                    return true;
