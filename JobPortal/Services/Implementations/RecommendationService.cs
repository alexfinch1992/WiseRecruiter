...
namespace JobPortal.Services.Implementations
{
    public class RecommendationService : IRecommendationService
    {
        ...
        public async Task<List<PendingRecommendationDto>> GetPendingStage1RecommendationsAsync()
        {
            var allRecs = await _context.CandidateRecommendations
                .Where(r => r.Stage == RecommendationStage.Stage1)
                .ToListAsync();

            Debug.WriteLine($"[Pending] Total Stage1 recs: {allRecs.Count}");
            Debug.WriteLine($"[Pending] Status=Submitted: {allRecs.Count(r => r.Status == RecommendationStatus.Submitted)}");
            Debug.WriteLine($"[Pending] Status!=Submitted: {allRecs.Count(r => r.Status != RecommendationStatus.Submitted)}");

            var pending = await _context.CandidateRecommendations
                .Include(r => r.Application)
                .Where(r => r.Stage == RecommendationStage.Stage1 && r.Status == RecommendationStatus.Submitted)
                .ToListAsync();
        ...
        public async Task<(CandidateRecommendation? Recommendation, bool IsApproved)> GetOrPrepareStage1RecommendationAsync(
            int applicationId,
