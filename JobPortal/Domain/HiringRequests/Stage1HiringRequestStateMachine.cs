...
namespace JobPortal.Domain.HiringRequests
{
    ...
    /// Also handles MoreInfoRequested ↔ Submitted round-trips initiated by the Talent Lead.
    /// </summary>
    public class Stage1HiringRequestStateMachine : IHiringRequestStateMachine<Stage1HiringRequestTransitionContext>
    {
        public bool CanTransition(HiringRequestStatus from, HiringRequestStatus to) =>
            (from, to) switch
            {
                ...
                (HiringRequestStatus.Submitted,         HiringRequestStatus.MoreInfoRequested)  => true,
                // Hiring Manager edits and resubmits after more info was requested
                (HiringRequestStatus.MoreInfoRequested, HiringRequestStatus.MoreInfoRequested)  => true,
                (HiringRequestStatus.MoreInfoRequested, HiringRequestStatus.Submitted)          => true,
                _ => false
            };
            ...
            HiringRequest entity, HiringRequestStatus to, Stage1HiringRequestTransitionContext ctx)
        {
            ...
            switch (to)
            {
                ...
                case HiringRequestStatus.MoreInfoRequested:
                    ...
                    // Apply content-field edits if present (MoreInfoRequested → MoreInfoRequested edit case).
                    // For Submitted → MoreInfoRequested (talent lead action), content fields are null.
                    ApplyContentFields(entity, ctx);
                    entity.UpdatedUtc = now;
                    break;
            }
        ...
        private static void ApplyContentFields(HiringRequest entity, Stage1HiringRequestTransitionContext ctx)
        {
