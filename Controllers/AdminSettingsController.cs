...
public class AdminSettingsController : Controller
{
    ...
    public AdminSettingsController(IFacetService facetService, IScorecardTemplateService templateService)
    {
    ...
    private async Task<EditTemplateFacetsViewModel?> BuildTemplateFacetEditorViewModel(int templateId, List<TemplateFacetInput>? postedFacets = null)
    {
        ...
        var assignedOrderByFacetId = assignedFacets
            .GroupBy(f => f.FacetId)
        ...
        return new EditTemplateFacetsViewModel
        {
            ...
            Facets = allFacets
                ...
                .Select(f => new TemplateFacetSelectionViewModel
                {
                    FacetId = f.Id,
                    FacetName = f.Name,
                    IsSelected = assignedOrderByFacetId.ContainsKey(f.Id),
                    DisplayOrder = assignedOrderByFacetId.TryGetValue(f.Id, out var order) ? order : f.DisplayOrder
                })
                .ToList()
        };
    }
